from utils import llm_call, prompt_chain_workflow, llm_router_call, ROUTING_MAP
...
if __name__ == "__main__":
    ...
    queries = [
        "리스본 여행 일정을 짜줘.",
        "1 더하기 2 는 뭐지?",
        "파이썬으로 웹 API 서버를 만들어줘."
    ]
    for query in queries:
        print(f"질문: {query}")
        query_type = llm_router_call(query)
        print(f"유형: {query_type}")
        final_llm_call = ROUTING_MAP[query_type](query)
        print(final_llm_call)
